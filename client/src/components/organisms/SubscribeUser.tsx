...
export default function SubscribeUser() {
  ...
  useEffect(() => {
    ...
    if (error) {
      const errorMessages = {
        ...
        "invalid-token":
          "Invalid verification link. Please try subscribing again.",
        "invalid-data":
          "Invalid subscription data. Please try subscribing again.",
        "expired-token":
          "Verification link has expired. Please subscribe again to get a new link.",
        "verification-failed":
          "Verification failed. Please try subscribing again.",
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await subscribe(email, firstName);

      if (data && data?.subscriber?.id) {
        setMessage({
          type: "success",
          text: "Verification email sent! Please check your inbox and click the confirmation link.",
        });
        setEmail("");
      } else {
        setMessage({
  ...
  return (
    <div className="max-w-2xl w-4/5 mx-auto space-y-8 p-8">
      ...
      <p className="text-sm md:text-lg font-montserrat text-center relative z-10">
        Get the latest news and updates from our club. We&apos;ll send you an
        email when we launch.
      </p>
